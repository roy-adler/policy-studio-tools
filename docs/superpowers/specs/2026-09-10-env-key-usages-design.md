...
When a leaf is selected in the ENV values editor, show where that key is referenced from Policy Studio policy files as `{{<env.path>.attributeValue}}`, and jump to the match on click.
...
2. **Match form:** Only `{{<dotted.path>.attributeValue}}`. Trim whitespace inside the braces. The ENV key is the dotted path with `.attributeValue` stripped. Exact key match only (no prefix matching). Other `{{…}}` placeholders are ignored.
...
6. **Out of v1:** Tree unused-highlight; interpolations that do not match an ENV key; placeholders other than `.attributeValue`; live watch of policy files (Reload rescans).
...
3. For each `.yaml` / `.yml` / `.xml` policy file, find `{{…attributeValue}}` matches; record range and 1-based line.
...
- A `{{id}}` / non-`attributeValue` placeholder that must not count
