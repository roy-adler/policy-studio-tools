...
When a leaf is selected in the ENV values editor, show where that key’s dotted path appears in sibling Policy Studio policy files, and jump to the match on click.
...
2. **Match form:** Substring search for the ENV leaf path in policy file text. `{{Service.X.serviceCert}}`, `{{Service.X.serviceCert.attributeValue}}`, and a bare `Service.X.serviceCert` all count. Do not match a longer identifier (`A.AAA` is not `A.AA`). `{{id}}` is not a usage of `A.AA`.
3. **Navigation:** Click a usage row → open the file and highlight the key path (same idea as jump-to-circuit).
...
| Unit | Responsibility |
|------|----------------|
| `findEnvAttributeUsages` | Discover sibling policy projects for an ENV root; walk policy files; find ENV leaf paths in file text |
...
3. For each `.yaml` / `.yml` / `.xml` policy file, find each ENV leaf path as text; record range covering that path and 1-based line.
...
- YAML / XML / bare text containing a known ENV key path
- A key in `values.yaml` with no occurrence (unused)
- A `{{id}}` string that must not count as a usage of `A.AA`

Unit tests: find key occurrences in text; used vs unused lookup; skip unreadable files; HTML contains badge, list, unused red state, and “Not used in any policy.”
